...
import me.phonemask.lib.api.Phone
import me.phonemask.lib.internal.core.Proxy
import me.phonemask.lib.internal.ext.*
import me.phonemask.lib.internal.io.FileReader
import me.phonemask.lib.internal.model.State
import me.phonemask.lib.internal.pattern.CountryPattern
import me.phonemask.lib.internal.ui.CountryPickerArguments
import me.phonemask.lib.internal.ui.CountryPickerBottomSheet
...
class PhoneNumberKit private constructor(
            ...
            object : MaskedTextChangedListener.ValueListener {
                override fun onTextChanged(
                    ...
                    if (state is State.Attached) {
                        var number = extractedValue.clearSpaces()
                        number = if (number.startsWithPlus()) number else number.prependPlus()
                        val countryCode = getCountryCodeByNumber(number ?: "")
                        if (state.country.code != countryCode) {
                            val country = countriesCache.findCountry(countryCode)
                            if (country != null) {
                                setCountry(country)
                            }
                        }
    ...
    private fun getCountryCodeByNumber(phone: String) : Int? {
        val codes = countriesCache.filter { phone.replace("+", "").startsWith(it.code.toString()) }
        return codes.maxOfOrNull { it.code }
    }
